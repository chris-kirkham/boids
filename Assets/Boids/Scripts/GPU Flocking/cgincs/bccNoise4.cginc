...
{
	float4 noise = Bcc4NoiseBase(coord);
	float theta = noise.x * noise.y * tau;
	float phi = noise.y * noise.z * tau;
	return float3(cos(theta), sin(theta), cos(phi));
}

float3 Bcc4NoiseBaseDirectional3D(float3 X)
{
	return PointOnUnitSphere(X);
}
