...
float Noise(float x, float y, float z)
{
	...
	float A = (perm[X] + Y) & 0xff;
	float B = (perm[X + 1] + Y) & 0xff;
	float AA = (perm[A] + Z) & 0xff;
	float BA = (perm[B] + Z) & 0xff;
	float AB = (perm[A + 1] + Z) & 0xff;
	float BB = (perm[B + 1] + Z) & 0xff;
