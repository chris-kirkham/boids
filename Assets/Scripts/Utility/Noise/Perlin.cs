...
/// <summary>
/// Perlin noise implementation - NOT MINE, FROM https://github.com/keijiro/PerlinNoise/blob/master/Assets/Perlin.cs, 
...
public static class Perlin
{
    ...
    public static Vector3 PointOnUnitSphere(Vector3 coord)
    {
        float theta = Noise(coord.y, coord.z) * tau;
        float phi = Noise(coord.x, coord.y) * tau;
        return new Vector3(Mathf.Cos(theta), Mathf.Sin(theta), Mathf.Cos(phi));
    }
    ...
    private static float FastFloor(float f)
    {
        return (int)(f + 32768f) - 32768;
    }

    private static int FastFloorToInt(float f)
    {
        return (int)(f + 32768f) - 32768;
    }
}
